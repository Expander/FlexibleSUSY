...
void FlavourMssmSoftsusy::calcDrBarPars() {
  ...
  for(i=1; i<=3; i++) {
     {
        const auto msu = extractFlavour(uSqMasses, uSqMixT.transpose(), i);
     ...
     {
        const auto msd = extractFlavour(dSqMasses, dSqMixT.transpose(), i);
     ...
     {
        const auto mse = extractFlavour(eSqMasses, eSqMixT.transpose(), i);
