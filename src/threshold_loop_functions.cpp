...
double phixyz(double x, double y, double z)
{
   ...
   // u > 1 && v < 1 --> x > z && z > y -> y < z < x -> we want x as the third argument
   // x * phi(x,y,z) = z*phi(z,y,x) -> phi(z,y,x) = x/z*phi(x,y,z)
   ...
   if (devu >= eps && devv >= eps && devm >= eps) {
      ...
      // u > 1 && v > 1 --> x > z && y > z -> z < y/x -> we want max(x,y) as the third argument
      else if (u > 1 && v > 1) {
      ...
      // u < 1 && v > 1 --> x < z < y --> we want y as the third argument
      else if (u < 1 && v > 1) {
         fac = z/y;
         my_x = z;
         my_y = x;
      ...
      return std::real(fac*1./lambda_c*(2.*std::log(xplus_c)*std::log(xminus_c)-std::log(u)*std::log(v)-2.*(dilog(xplus_c)+dilog(xminus_c))+sqr(PI)/3.));
   }
   ...
   // u = 1
   if (devu < eps) {
      // if v > 1 then y> z (= x) we have again to reshuffle the argument: here we have phi(z,y,z) with z < y and we want phi(z,z,y)
      if (v > 1) {
         fac = z/y;
         my_x = z;
         my_y = x; // should be equal to
         my_z = y;
         return std::real(fac*1./complex_sqrt(1.-4.*my_x/my_z)*(sqr(PI)/3.+2.*sqr(std::log(0.5 - 0.5*complex_sqrt(1.-4.*my_x/my_z)))
                                                                - sqr(std::log(my_x/my_z)) - 4.*dilog(0.5*(1.-complex_sqrt(sqr(1-2.*my_x/my_z)-4.*sqr(my_x)/(sqr(my_z)))))
                             ));
      }
      // phi(z,y,z) with z > y, just need to reshufle to phi(y,z,z), i.e. do nothing since they are the same
      else {
         fac = 1.;
         my_x = y;
         my_y = x;
         my_z = z; // should be equl to my_y
         return std::real(fac*1./(3.*complex_sqrt(my_x*(my_x-4*my_z)/(sqr(my_z))))*(sqr(PI)+6.*std::log((my_x - my_z*complex_sqrt(my_x*(my_x-4.*my_z)/sqr(my_z)))/(2.*my_z))
                                                                                    *std::log(1. - my_x/(2.*my_z) - 0.5*complex_sqrt((sqr(my_x)-4.*my_x*my_z)/sqr(my_z)))
                                                                                    -6.*dilog(0.5*(2.-complex_sqrt(sqr(my_x)/sqr(my_z)-4.*my_x/my_z)-my_x/my_z))
                                                                                    -6.*dilog(0.5*(-complex_sqrt(sqr(my_x)/sqr(my_z)-4.*my_x/my_z)+my_x/my_z))
                             ));
      }
   }
   ...
   // v = 1
   if (devv < eps) {
      // if u > 1 we have again to reshuffle the arguments: here we start with phi(x,z,z) with z < x and we want phi(z,z,x)
      if (u > 1) {
         fac = z/x;
         my_x = z;
         my_y = y;
   ...
   if (devm < eps) {
      // if (v = ) u > 1, we are in the case phi(x,x,z) with x > z. We have to reshufle to phi(z,x,x)
      if (u > 1) {
         fac = z/x;
         my_x = z;
         my_y = y;
