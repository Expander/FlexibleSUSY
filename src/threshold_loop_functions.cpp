 ...
 * \endcode
 *
 * where PolyLog(x) = PolyLog(x,2) = Spence(1-x) dilogarithm function.
 *
 * @return fin(m12, m22)
 */
double fin(double mm1, double mm2)
{
   ...
   if (mm1 > mm2) {
      return -7./2. - (7.*mm2)/(2.*mm1) +
         dilog(mm2/mm1) - (mm2*dilog(mm2/mm1))/mm1 -
         (3.*mm2*log(mm1))/mm1 - log(mm1)*log(mm1 - mm2) +
         (mm2*log(mm1)*log(mm1 - mm2))/mm1 + (3.*mm2*log(mm2))/mm1 -
         log(mm1)*log(mm2) + (2.*mm2*log(mm1)*log(mm2))/mm1 +
         log(mm1 - mm2)*log(mm2) - (mm2*log(mm1 - mm2)*log(mm2))/mm1 - sqr(PI)/4. +
         (mm2*sqr(PI))/(12.*mm1) + sqr(log(mm1)) - (3.*mm2*sqr(log(mm1)))/(2.*mm1) -
   ...
   if (mm1 < mm2) {
      return -7./2. - (7.*mm2)/(2.*mm1) -
         ...
         (mm2*sqr(PI))/(4.*mm1) + sqr(log(mm1))/2. - (mm2*sqr(log(mm1)))/mm1 -
         sqr(log(mm2))/2.;
   }
}
