...
\item \emph{Low-scale constraint:} The low-scale constraint is the
  ...
  is fixed to be the $Z$ pole mass scale $M_Z^\pole$.  Optionally the
  Yukawa couplings $y_f$ ($f=u,d,e$) can be calculated automatically
  from the known Standard Model fermion masses $m_f$ by setting their
  values to \code{Automatic}.  This automatic calculation is explained
  in \secref{sec:calculation-of-yukawa-couplings}.
\end{itemize}
%
...
SUSYScaleInput = {
  ...
  \secref{sec:calculation-of-yukawa-couplings}.  To enable the
...
parameters, see \secref{sec:calculation-of-gauge-couplings}.
...
the Standard Model and user-defined input paramters and numerically
...
then show two concrete boundary conditions, which are always imposed:
The matching of the model parameters to the Standard Model and the
...
classes now allow to use renormalisation group running of all SUSY
model parameters.
...
for convergence after each iteration.  This approach is decribed in
