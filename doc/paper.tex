...
allowing fast and reliable exploration of the sparticle spectrum and
mixings that can be obtained from particular choices of breaking
mechanism inspired boundary conditions and specified parameters. There
are also two public spectrum generators
\cite{Ellwanger:2006rn,Allanach:2013kza} for the next to minimal
supersymmetric standard model (NMSSM) \cite{NMSSM} .

None of the fundamental motivations of supersymmetry require
minimality, and specific altternatives to (or extensions of) the MSSM
are motiovated by, for example,the $\mu-problem$ of the
...
the fundamental questions which motivated BSM physics unanswered.

 Secondly ATLAS and CMS discovered\cite{ATLAS:2012ae,
   Chatrchyan:2012tx} a light Higgs of $125$ GeV, within the mass
 range that could be accommodated in the MSSM but requiring stops
 which are significantly heavier than both the direct collider limits
 and indirect limits that appears in constrained models from the
 significantly higher limits on first and second generation squarks.
 Alternatively it is possible that a $125$ GeV Higgs in non-minimal
 models may not require such heavy stops, indeed a number of such
 models have been constructed to do exactly this by having new tree
 level contributions to the Higgs mass.

These motivate both exploring non-minimal SUSY models which ameliorate
the naturaless problems introduced, by, for example, raising the tree
level Higgs mass, and models developed with a fresh persp[ective,
...
flexible as possible to accomodate different types of extensions and
due to it's modular nature it is easy to modify, add new featuires to
or combine with other programs.
