...
To realized this, all (derived) constraint objects are intentionally
...
adjust initial guesses specified in the FlexibleSUSY modle file, such
as changing the choice of HighScaleFirstGuess or altering
HighScaleMinimum and HighScaleMaximum which can be used to push the
iteration back towards where the solution should be if it gets off
track.

For experienced users the clear code structure also allows the
possibility of direct adaption of the code.  For example in the case
of the E6SSM we observed that for some points the presence of new
Yuwawas had a big impact on the RGEs for the VEVS.  When the user
chooses to set these Yukawas at the GUT scale, then on the initial run
up to the GUT scale they will be set to zero.  As a result the VEV
RGEs on the run up are quite different to the VEV RGEs on the run down
so that the VEVs used at the SUSY scale are far from the correct
values and this can create an instability leading eventually to
non-perturbative in the couplings.  This can be fixed by adapting the
algorithm so that thge VEVs are stored at the SUSY scale.  However a
simpler, if less clean, fix is to directly modify the generated code
by adding three lines to the file
\code{<model>_two_scale_susy_parameters.cpp} just before the beta
functions are returned in method
\code{<model>_susy_parameters::calc_beta()}.

\code{ 
if(get_scale()  > 3.0e3) beta_vd =0;
if(get_scale()  > 3.0e3) beta_vu =0;
if(get_scale()  > 3.0e3) beta_vs =0;
return E6SSMnew_susy_parameters(get_scale(), get_loops(), get_thresholds(), input,
beta_Yd, beta_Ye, beta_Kappa, beta_Lambda12, beta_Lambdax, beta_Yu, beta_MuPr, beta_g1, beta_g2, beta_g3, beta_g1p, beta_vd, beta_vu, beta_vs);
}

Finally instead of tinkering with the two-scale solver one may wish to replace it entirely. the modular design of FlexibleSUSY allows for the solver
for the boundary value problem to be replaced. An alternative solver
with better convergence properties (at the expense of slower speed) is
...
\fs generated NUH \ESSM has also been compared against a handwritten
spectrum generator for a constrained version of the \ESSM
\cite{Athron:2009bs,Athron:2012pw}.  The RGEs could be compared
directly in unit tests and were found to match.  The handwritten code
doesn't include full one loop self energies or tadpoles so tests on
these were not carried.  Although the generators assume different
constraints they could be compared by using the output of the
CE$_6$SSM as input to \fs and the spectra were found to be in
reasonable agreement, given the different levels of precision.
...
We have presented \fs, a Mathematica package, which generates fast and
...
introducing couplings with a single family index; Philip Diessner for
testing and identifying bugs in the MRSSM model file; Ulrik Günther
for compilation tests on Mac OS X\@ and Dylan Harries for spotting
a bug in the configuration script.
