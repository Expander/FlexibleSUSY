...
This avoids underflows or overflows of the scale value during the
iteration.  This is especially useful in models where the iteration is
very unstable and the value of the scale is very sensitive to the
model parameters.
%
\begin{itemize}
...
created by \fs, the model file variable \code{PotentialLSPParticles}
must be set to a list of SUSY particles which are potential LSPs.  In
the model file example above, the particles \code{Chi}, \code{Cha},
...
the soft-breaking parameters in terms of all model parameters.
This hierarchy allows to
%
\begin{enumerate}
\item use the RGE running of all model parameters via the common
  \code{Beta_function} interface and to
\item run the susy parameters independently of the soft-breaking
  parameters.
\end{enumerate}
...
The $\beta$-functions for the model parameters together with the
boundary conditions for a boundary value problem.  The default
two-scale RG solver tries to find the full set of model parameters
...
the constraints and checking for convergence after each iteration.
This approach is decribed in \cite{Barger:1993gh} for the MSSM and is
widely implemented in SUSY spectrum generators.

The in more detail the two-scale algorithm used in \fs works as
follows, see \figref{fig:two-scale-algorithm}:
%
\subparagraph{Initial guess:} The RG solver starts to guess all model
parameters at the low-scale.
%
\begin{enumerate}
\item At the $M_Z$ scale the susy model gauge couplings $g_{1,2,3}$
  are set to the known Standard Model values (ignoring threshold
  corrections).
\item Then, the user-defined low-scale boundary conditions (defined in
  \code{LowScaleInput}) are imposed.
\item The Yukawa couplings $y_{u,d,e}$ of the susy model are set to
  the known Standard Model Yukawa couplings.
...
\item \label{rge-step-one} All model parameters are run to the
  ...
  \begin{enumerate}
  \item The \DRbar\ mass spectrum is calculated.
  \item The low-scale is recalculated.
  \item The \DRbar\ gauge and Yukawa couplings $g_{1,2,3}(M_Z)$,
    $y_{u,d,e}(M_Z)$ of the susy model are calculated using threshold
    corrections.
...
\item Run all model parameters to the high-scale (\code{HighScale})
  \begin{enumerate}
  \item Recalculate the high-scale
  \item Apply the high-scale constraint (\code{HighScaleInput})
  \end{enumerate}
\item Run model parameters to the susy-scale (\code{SUSYScale})
  \begin{enumerate}
  \item Calculate the \DRbar\ mass spectrum
  \item Recalculate the susy-scale
  \item Apply the susy-scale constraint (\code{SUSYScaleInput})
  \item Solve the EWSB eqs.\ at the loop level
  \end{enumerate}
...
We compare the run-time of five different CMSSM spectrum generators:
%
\begin{itemize}
\item FlexibleSUSY-FV (version 1.0.0), which is based on SARAH's MSSM
  model file that allows flavour violation,
\item FlexibleSUSY-NoFV (version 1.0.0), without flavour violation
\item Softsusy (version 3.4.0), without flavour violation
\item SPheno (3.2.4), without flavour violation
\item SPhenoMSSM (generated with SARAH 4.1.0 and linked against SPheno
  3.2.4), which uses SARAH's MSSM model file that includes flavour
  violation.
\end{itemize}
%
FlexibleSUSY and Softsusy were compiled with g++ 4.8.0 and Intel ifort
13.1.3 20130607.  SPheno and SPhenoMSSM were compiled with Intel ifort
13.1.3 20130607\footnote{We are using Intel's ifort compiler, instead
  of gfortran, because it decreases the run-time of SPheno and
  SPhenoMSSM by approximately a factor $1.5$.}.  We're generating
$2\cot 10^{4}$ random CMSSM parameter points with $m_0\in [50,1000]$
...
\tablename~\ref{tab:run-time-comparison}.  The first column shows the
run-time on a Intel Core2 Duo (P8600) where only one core was enabled.
The second column shows the run-time on a Intel Core2 Duo (P8600)
where both cores were enabled.  In the third column a Intel Xeon
(L5640) was used, which has $6$ CPU cores.
%
\begin{table}[tbh]
  \centering
  \begin{tabular}{llll}
  ...
  \caption{Average run-time of CMSSM spectrum generators for
    for random parameter points.  The upper }
  \label{tab:run-time-comparison}
\end{table}

Under both the non-flavor violating spectrum generators
FlexibleSUSY-NoFV, Softsusy and SPheno (first three rows) as well as
the flavor violating ones FlexibleSUSY-NoFV and SPhenoMSSM (4th and
5th row) we find that \fs is significantly fastest.  We also find that
increasing the number of CPU cores reduces the run-time of \fs.  The
reason is that \fs calculates each pole mass in a separate thread, and
therefore benefits from multi-core CPUs.  Under the hand-written CMSSM
spectrum generators SPheno and Softsusy we find that SPheno is fastest
on average.
