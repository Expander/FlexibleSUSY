...
the pole masses at the one-loop level, which means finding the
eigenvalues $p$ that solve the following equations:
%
\begin{align}
  &\text{scalars } \phi:  & 0 &= p^2\unity - m^2_{\phi,1L}(p^2), \\
  &\text{fermions } \xi: & 0 &= p\unity - m_{\xi,1L}(p^2),
\end{align}
%
where the one-loop mass matrices are given by
%
\begin{align}
  &\text{scalars } \phi: &
...
$m_{\phi,1L,\text{low}}$ is calculated exactly once as
%
\begin{align}
  (m_{\phi,1L,\text{low}})_{ij} = (m_{\phi,1L}(p^2 = m_{\phi_i} m_{\phi_j}))_{ij} ,
\end{align}
%
where $m_{\phi_i}$ is the $i$th mass eigenvalue of the tree-level mass
matrix $m_\phi$.  Afterwards $m_{\phi,1L,\text{low}}$ is diagonalized
and the eigenvalues are interpreted as pole masses $m_{\phi_i}^\pole$.
...
the one-loop mass matrix $m_{\phi,1L,\text{medium}}$ is calculated $n$
times as
%
\begin{align}
  (m_{\phi,1L,\text{medium}})_{ij}^{(k)} = (m_{\phi,1L}(p^2 = m_{\phi_k}^2))_{ij} ,
  \qquad k = 1,\ldots,n ,
\end{align}
%
where $m_{\phi_k}$ is the $k$th mass eigenvalue of the tree-level mass
matrix $m_\phi$.  Afterwards, each mass matrix
$(m_{\phi,1L,\text{medium}})^{(k)}$ is diagonalized and the $k$th
eigenvalue is interpreted as pole mass $m_{\phi_k}^\pole$.
...
$m_{\phi,1L,\text{high}}$ is diagonalized $n$ times as in the case of
\code{MediumDiagonalizationPrecision}, resulting in $n$ pole masses
$m_{\phi_k}^\pole$ ($k = 1,\ldots,n$).  Afterwards, the
diagonalization is repeated, this time using the so calculated pole
masses $m_{\phi_k}^\pole$ for the momentum calculation $p^2 =
(m_{\phi_k}^\pole)^2$, until convergence is reached.
...
P.~Slavich from \cite{slavich-degrassi} to add two loop corrections of
$\oatas$, $\oabas$, $\oatq$, $\oabatau$, $\oabq$, $\oatauq$ and
$\oatab$ as follows... {\color{red} This is not yet implemented!}
