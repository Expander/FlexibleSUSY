...
% where $\mu$ is chosen to be fixed by the EWSB equations
...
\begin{table}[tbh]
  ...
  \begin{tabular}{llll}
    ...
    \midrule
    $m_h$
    & $1441\unit{GeV}$ ($0.96\unit{ms}$)
    & $128\unit{GeV}$ ($2.53\unit{ms}$)
    & $126\unit{GeV}$ ($9.79\unit{ms}$) \\
    $m_{\tilde{g}}$
    & $10167\unit{GeV}$ ($0.10\unit{ms}$)
    & $10167\unit{GeV}$ ($0.10\unit{ms}$)
    & $10251\unit{GeV}$ ($0.41\unit{ms}$) \\
    $m_{\tilde{u}_1}$
    & $8702\unit{GeV}$ ($2.57\unit{ms}$)
    & $8714\unit{GeV}$ ($15.7\unit{ms}$)
    & $8734\unit{GeV}$ ($60.9\unit{ms}$) \\
    \bottomrule
  \end{tabular}
...
\end{itemize}
%
For the run-time comparison the CKM matrix is set to unity, all CP
violating phases are set to zero and R-parity violation is disabled.
\fs and Softsusy are compiled with g++ 4.8.0 and Intel ifort 13.1.3
20130607.  SPheno and SPhenoMSSM are compiled with Intel ifort 13.1.3
20130607.\footnote{Intel's ifort compiler decreases the run-time of
  SPheno and SPhenoMSSM by approximately a factor $1.5$, compared to
  gfortran.}  We're generating $2\cdot 10^{4}$ random CMSSM parameter
points with $m_0\in [50,1000]\unit{GeV}$, $m_{1/2}\in
...
\begin{table}[tbh]
  ...
  \caption{Average run-time of CMSSM spectrum generators
    ...
    spectrum generators which disallow sfermion flavour violation.  Rows
    $4$--$5$ contain sfermion flavour violating spectrum generators, based
