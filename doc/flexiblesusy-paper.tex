...
CP-even Higgs tadpoles and self-energies.  For MSSM-like models (with
two CP-even Higgs bosons, one CP-odd Higgs boson, one neutral
Goldstone boson) routines dor caluclating these corrections will be
generated by setting \code{UseHiggs2LoopMSSM = True} in the model file
and by defining the effective $\mu$-term \code{EffectiveMu = \\[Mu]}.
This will add the zero-momentum corrections of the order $O(y_t^4 +
y_b^2 y_t^2 + y_b^4)$, $O(y_t^2 g_3^2)$, $O(y_b^2 g_3^2)$,
$O(y_\tau^4)$ from
\cite{Degrassi:2001yf,Brignole:2001jy,Dedes:2002dy,Brignole:2002bz,Dedes:2003km}.
...
correction for that order in the NMSSM, but can be a good approximation
when singlet mixing is very small.
...
{\color{red} To include all two loop corrections which have been
  generated the user may simply set entry 4 of the FlexibleSUSY block
  in the SLHA file to 2.  The user may also want to include only some of
  the corrections generated, for example in the NMSSM the user may decide not to use the partial corrections at order $\oatq$, $\oabq$, $\oatauq$ and $\oatab$ which have been computed in the MSSM. In that case the entries 9,10,11 and 12 may be used to switch on and off corrections at specific orders, 1 meaning the correction is included and 2 means the correction is not.  Alex: can you check this part matches what you did and maybe fox details a little and also we should decide if it belongs here or if describe SLHA stuff later.}

Since the Higgs mass is a very important measurement and
the two-loop corrections can be larger than the current experimental
error \cite{Degrassi:2009yq} we recommend to set these switches to
...
there are other potentially large two-loop corrections.  For models
...
\bibitem{Nevzorov:2013tta} 
  ...
  %%CITATION = ARXIV:1308.1021;%%
  %2 citations counted in INSPIRE as of 13 Jun 2014

%\cite{Nevzorov:2013ixa}
\bibitem{Nevzorov:2013ixa} 
  ...
  %%CITATION = ARXIV:1309.4738;%%
  %1 citations counted in INSPIRE as of 13 Jun 2014
