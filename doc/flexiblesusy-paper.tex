...
The standard input and output of the generated code is given in SLHA
format and an extension of this to incorporate new blocks for models
which go beyond the MSSM and NMSSM. We use the well known SLHAea
\cite{slhaea} to create these SLHA files and the extra blocks may be chosen
by the user. Since the SLHA standard is intended precisely for the
purpose, MSSM and NMSSM codes should be compliant with this, and we
have tested that HDECAY\cite{hdecay} can understand the SLHA output of
FlexibleSUSY.
...
called \code{MSSM}), the NMSSM in it's $Z_3$-symmetric form (called \code{NMSSM}), $Z_3$-violating NMSSM (\code{SMSSM}), the USSM (\code{UMSSM}),
...
when singlet mixing is very small\footnote{These corrections may be disabled in the SLHA file, as described in section \secref{sec:PoleMasses}.}.
