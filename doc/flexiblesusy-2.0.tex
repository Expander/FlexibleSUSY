...
\begin{table}[tbh!]
  ...
  \begin{tabular}{llll}
    ...
    14 & \code{Mv2} & $0$ & 2nd lightest neutrino pole mass, $M_{\nu_2}/\text{GeV}$ \\
...
\begin{table}
  ...
  \begin{tabularx}{1.2\textwidth}{lllX}
    ...
    enable/disable 2-loop corrections $O(\atau\atau)$ to $M_{h,H,A}$\\
...
\item[\texttt{FlexibleSUSY[24]},] \texttt{thresholdCorrections}: With
  ...
  \begin{example}[label=ex:HSSUSY_thresholds]
    ...
    corrections globally.  Note that the digit at the $6$th position
...
\item[\texttt{FlexibleSUSY[26]},] \texttt{higgs3loopCorrectionAtAsAs}:
  ...
  Higgs pole mass(es) are enabled in the \fs model file.  See
