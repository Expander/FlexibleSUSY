...
# This script generates a CMSSM spectrum and writes it to
# a SLHA file.  The EWSB output parameters from this point are then
# used as inputs for the CMSSM solved using the semi-analytic solver.
# After running the semi-analytic CMSSM we compare that the
...
two_scale_slha="$BASEDIR/../model_files/CMSSM/LesHouches.in.CMSSM"
...
match_input_blocks() {
    ...
    $awk_cmd -f "$print_block" -v block=FlexibleSUSY "$_input_file" \
        | $awk_cmd -f "$remove_block" -v block=FlexibleSUSY -v entry=2 \
             > "$_output_file"
    echo "    2   2" >> "$_output_file"

    $awk_cmd -f "$remove_block" -v block=FlexibleSUSY "$_input_file" \
        | $awk_cmd -f "$remove_block" -v block=MINPAR -v entry=1 \
        | $awk_cmd -f "$remove_block" -v block=MINPAR -v entry=4 \
                   >> "$_output_file"

    echo "Block EXTPAR" >> "$_output_file"
    echo "   23   $_mu_val" >> "$_output_file"
}

# generate point using two-scale solver
...
if test ! -r "$two_scale_output"; then
    echo "Error: generated CMSSM spectrum not found: $two_scale_output"
    exit 1
fi

# remove comments from the two-scale output spectrum
$sed_cmd -i~ -e '/^ *#/d' $two_scale_output

# construct matching input point for the semi-analytic solver
mu_sol=$(awk -f "$print_block" -v block=HMIX "$two_scale_output" \
             | awk -f "$print_block_entry" -v entries=1)
match_input_blocks $two_scale_input $semi_analytic_input $mu_sol

# generate point using semi-analytic solver
echo -n "running semi-analytic solver ..."
$semi_analytic_exe --slha-input-file=$semi_analytic_input --slha-output-file=$semi_analytic_output
echo "done"
echo "CMSSMSemiAnalytic SLHA input file:  $semi_analytic_input"
echo "CMSSMSemiAnalytic SLHA output file: $semi_analytic_output"

# remove comments from semi-analytic output spectrum
...
echo ""
echo "comparing files $two_scale_output and $semi_analytic_output ..."
echo "required relative agreement: $rel_error"

diff=`$numdiff_cmd\
 --absolute-tolerance=1.0e-12\
 --relative-tolerance=$rel_error\
 $two_scale_output $semi_analytic_output`
...
if [ -n "$diff_without_comments" ]; then
    echo "Error: difference between $two_scale_output and $semi_analytic_output larger than $rel_error"
    echo "$diff"
    echo ""
