...
EOF
    ...
    rm -rf SPheno.spc.MRSSM tmp.in
    echo "$slha_input" > tmp.in
    cat <<EOF >> tmp.in
...
EOF
    ...
    $spectrum_generator tmp.in > /dev/null 2>&1

    slha_output=
    if test "x$?" = "x0" -a -f "SPheno.spc.MRSSM" ; then
        slha_output=$(cat SPheno.spc.MRSSM)
    fi
